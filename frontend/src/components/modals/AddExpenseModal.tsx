...
export function AddExpenseModal({
  open, onClose, onSubmit, members, currentUserId, isLoading
  ...
  // When switching to custom mode, pre-populate split rows
  useEffect(() => {
    if (splitMode === 'custom') {
      replace(members.map(m => ({ user_id: m.user_id, username: m.username, amount: '' })))
    }
  }, [splitMode, members, replace])
  ...
  const autoSplit = () => {
    ...
    replace(members.map(m => ({ user_id: m.user_id, username: m.username, amount: each })))
  }
          ...
          <div className="flex gap-2">
            {(['equal', 'custom'] as const).map(mode => (
              <button
                key={mode}
                type="button"
                onClick={() => setValue('split_mode', mode)}
